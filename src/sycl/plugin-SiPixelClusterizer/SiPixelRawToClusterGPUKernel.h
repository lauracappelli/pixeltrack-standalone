...
namespace pixelgpudetails {
  ...
  inline uint32_t pack(uint32_t row, uint32_t col, uint32_t adc) {
    ...
    adc = sycl::min(adc, (const unsigned int)(thePacking.max_adc));
  ...
  public:
    ...
    std::pair<SiPixelDigisCUDA, SiPixelClustersCUDA> getResults() {
      ...
      // the CUDA streams are cached within the cms::sycltools::StreamCache, but it is
  ...
  private:
    ...
    cms::sycltools::host::unique_ptr<uint32_t[]> nModules_Clusters_h;
